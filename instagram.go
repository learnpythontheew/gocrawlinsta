...
type mainPageData struct {
	...
	Rhxcsrft  string `json:"rhx_csrft"`
	EntryData struct {
		ProfilePage []struct {
			Graphql struct {
				User struct {
...
func main() {
	...
	c.OnRequest(func(r *colly.Request) {
		...
		// }
		if r.Ctx.Get("csrft") != "" {
			csrft := fmt.Sprintf("%s:%s", r.Ctx.Get("csrft"), r.Ctx.Get("variables"))
			h := md5.New()
			h.Write([]byte(csrft))
			csrftHash := fmt.Sprintf("%x", h.Sum(nil))
			r.Headers.Set("x-csrftoken", csrftHash)
			fmt.Print(csrft)
		}
	})
	...
	c.OnHTML("html", func(e *colly.HTMLElement) {
		...
		if page.Graphql.User.Media.PageInfo.NextPage {
			...
			e.Request.Ctx.Put("csrft", data.Rhxcsrft)
			e.Request.Visit(u)
		}
	})
